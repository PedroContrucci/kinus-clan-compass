...
const Cla = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-36 lg:pb-24">
        ...
        <div className="px-4 py-3">
          ...
          <div className="mb-3 flex gap-2">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar lugares em esta cidade..." value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="border-border bg-card pl-10" />
                ...
                <div>
                  ...
                  {filtered.sharedTrips.length === 0 ? <p className="text-sm text-muted-foreground">Ninguém do clã compartilhou um roteiro de {city} ainda.</p> : (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {(topOnly ? filtered.sharedTrips.slice(0, 5) : filtered.sharedTrips).map((trip, index) => <SharedTripCard key={trip.id || index} trip={trip} open={openShared === index} onToggle={() => setOpenShared(openShared === index ? null : index)} catalog={cityData.activities} onAdd={setAddTarget} />)}
                    </div>
                  )}
                </div>
              </section>
      ...
      <HotelDetailDrawer open={!!selectedHotel} onClose={() => setSelectedHotel(null)} hotel={selectedHotel} city={city} onSelect={(hotel) => { setAddTarget({ kind: 'hotel', hotel }); setSelectedHotel(null); }} />
...
function CatalogCardView({ card, city, stats, onActivity, onHotel, onMichelin, onAdd }: { card: CatalogCard; city: string; stats: Map<string, ClaStat>; onActivity: (activity: SuggestedActivity) => void; onHotel: (hotel: CuratedHotel) => void; onMichelin: (restaurant: MichelinRestaurant) => void; onAdd: (target: AddTarget) => void }) {
  if (card.kind === 'activity') {
    ...
    return <article className="overflow-hidden rounded-lg border border-border bg-card"><button type="button" onClick={() => onActivity(card.activity)} className="block w-full text-left"><ClaLazyImage name={card.activity.name} city={city} className="aspect-[16/9] overflow-hidden" /><div className="space-y-2 p-4"><h3 className="font-['Outfit'] font-semibold text-foreground">{card.activity.name}</h3><p className="text-xs text-muted-foreground">{card.activity.neighborhood}</p><div className="flex flex-wrap gap-2 text-[11px]"><span className="rounded-full bg-muted px-2 py-1 text-muted-foreground">{categoryOf(card.activity) === 'restaurant' ? 'Restaurante' : categoryOf(card.activity) === 'beach' ? 'Praia' : 'Experiência'}</span><span className="text-muted-foreground">Google {rating(card.activity.rating)}</span>{social.ups > 0 && <span className="text-emerald-400">Clã 👍 {social.ups}</span>}<span className="text-foreground">{brl(card.activity.estimatedCostBRL)}</span></div></div></button><div className="border-t border-border px-4 py-3"><Button variant="ghost" size="sm" className="h-8 px-0 text-xs text-primary" onClick={() => onAdd({ kind: 'activity', activity: card.activity })}>➕ Adicionar à minha viagem</Button></div></article>;
  }
  if (card.kind === 'hotel') {
    return <article className="overflow-hidden rounded-lg border border-border bg-card"><button type="button" onClick={() => onHotel(card.hotel)} className="block w-full text-left"><ClaLazyImage name={card.hotel.name} city={city} className="aspect-[16/9] overflow-hidden" /><div className="space-y-2 p-4"><h3 className="font-['Outfit'] font-semibold text-foreground">{card.hotel.name}</h3><p className="text-xs text-muted-foreground">{card.hotel.zone} · {TIER_LABEL[card.hotel.tier] ?? card.hotel.tier}</p><div className="flex flex-wrap gap-2 text-[11px]"><span className="text-muted-foreground">Google {rating(card.hotel.rating)}</span><span className="text-foreground">{card.hotel.priceRangeBRL}</span></div></div></button><div className="border-t border-border px-4 py-3"><Button variant="ghost" size="sm" className="h-8 px-0 text-xs text-primary" onClick={() => onAdd({ kind: 'hotel', hotel: card.hotel })}>Usar este hotel</Button></div></article>;
  }
  if (card.kind === 'michelin') {
    return <button type="button" onClick={() => onMichelin(card.restaurant)} className="overflow-hidden rounded-lg border border-border bg-card text-left transition-colors hover:border-primary/40"><ClaLazyImage name={card.restaurant.name} city={city} className="aspect-[16/9] overflow-hidden" /><div className="space-y-2 p-4"><h3 className="font-['Outfit'] font-semibold text-foreground">{card.restaurant.name}</h3><p className="text-xs text-muted-foreground">{card.restaurant.neighborhood || 'Bairro não informado'} · {card.restaurant.cuisine}</p><div className="flex gap-2 text-[11px] text-amber-400"><span>{'⭐'.repeat(card.restaurant.stars)} Michelin</span><span className="text-muted-foreground">{card.restaurant.priceRange}</span></div></div></button>;
  }
  const social = scoreFor(card.activity.id, card.activity.rating, stats);
  return <article className="overflow-hidden rounded-lg border border-border bg-card"><button type="button" onClick={() => onActivity(card.activity)} className="block w-full text-left"><ClaLazyImage name={card.activity.name} city={city} className="aspect-[16/9] overflow-hidden" /><div className="space-y-2 p-4"><p className="text-xs font-medium text-primary">Dica de {card.activity.name}</p><h3 className="font-['Outfit'] text-sm font-semibold leading-relaxed text-foreground">{card.tip}</h3><p className="text-xs text-muted-foreground">{card.activity.neighborhood}</p><div className="flex gap-2 text-[11px]"><span className="text-muted-foreground">Google {rating(card.activity.rating)}</span>{social.ups > 0 && <span className="text-emerald-400">Clã 👍 {social.ups}</span>}</div></div></button></article>;
}
