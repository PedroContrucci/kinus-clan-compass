...
import { inferAirportCode } from '@/components/cockpit/DraftCockpit';
    ...
    registerActionHandlers({
      ...
      adicionar_atividade: ({ dia, atividade, horario }) => {
        ...
        const suggested = catalog.find(s => s.name.toLowerCase() === needle)
          || catalog.find(s => s.name.toLowerCase().includes(needle))
          || catalog.find(s => needle.includes(s.name.toLowerCase()));
        if (!suggested || !horario) return null;
                          ...
                          <div className="flex-1 pb-4">
                            ...
                            {activity.status !== 'confirmed' && activity.status !== 'cancelled' && (
                              <div className="flex gap-2 mt-3 flex-wrap">
                                ...
                                <button
                                  onClick={(e) => { e.stopPropagation(); setSwapModal({ dayIndex, actIndex, activity }); }}
                                  className="flex items-center gap-1 px-3 py-1.5 bg-[#0f172a] border border-[#334155] rounded-lg text-xs text-[#f8fafc] hover:border-primary/50 transition-colors"
                                >
                                  🔄 Trocar
                                </button>
